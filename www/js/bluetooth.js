...
export async function testPrinter(printer){const p=plugin();if(!p)throw new Error('Bluetooth direct hanya tersedia di APK Transaku.');await p.print({address:printer.address,mode:printer.mode||'auto',encoding:printer.encoding||'CP437',data:'\x1B\x40TRANS AKU\nTest printer berhasil\n\n\n'})}
